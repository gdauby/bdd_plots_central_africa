...
.traits_to_genera_aggreg <- function(dataset, wd_fam_level_add = wd_fam_level) {
  ...
  if (any(class(all_val_sp$traits_categorical) == "data.frame")) {
    ...
    traits_idtax_char <-
      ...
      dplyr::select(-rn) %>%
      left_join(all_val_sp$traits_categorical %>%
  ...
  if (any(class(all_val_sp$traits_numeric) == "data.frame")) {
    ...
    #   ) %>%
    #   dplyr::select(-rn) %>%
    #   dplyr::left_join(all_val_sp$traits_numeric %>%
    #                      dplyr::select(idtax, tax_gen),
    #                    by = c("idtax" = "idtax"))
    # 
    ...
    traits_idtax_num <-
      pivot_numeric_traits_generic(
        data = all_val_sp$traits_raw %>%
          dplyr::filter(valuetype == "numeric", !is.na(tax_gen)),
        id_col = "tax_gen",
        include_stats = TRUE,
        include_id_measures = T,
        name_prefix = "taxa_"
      )
    # %>%
    #   dplyr::left_join(all_val_sp$traits_numeric %>%
    ...
    wd_plot_level <- 
      ...
      dplyr::summarise(taxa_wood_density_mean_plot_level = mean(taxa_mean_wood_density, na.rm = T),
                       taxa_wood_density_sd_plot_level = mean(taxa_sd_wood_density, na.rm = T))
    ...
    dataset_pivot_wider_num <- 
      ...
      dplyr::mutate(taxa_mean_wood_density = 
                      ifelse(is.na(taxa_mean_wood_density),
                             taxa_wood_density_mean_plot_level,
                             taxa_mean_wood_density),
                    taxa_sd_wood_density = 
                      ifelse(is.na(taxa_sd_wood_density),
                             taxa_wood_density_sd_plot_level,
                             taxa_sd_wood_density),
                    ...
                    source_taxa_mean_wood_density = 
                      ifelse(is.na(source_taxa_mean_wood_density),
                             "plot_mean",
                             source_taxa_mean_wood_density)) %>%
      dplyr::select(-taxa_wood_density_mean_plot_level, taxa_sd_wood_density)
