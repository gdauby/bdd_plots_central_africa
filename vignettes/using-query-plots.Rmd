...
To include individual tree measurements:
...
# By coordinates (bounding box)
# plots_in_bbox <- query_plots(
#   lat_min = -2, lat_max = 2,
...
Enrich tree data with species-level traits from the taxa database:

```{r with-traits}
trees_with_traits <- query_plots(
  plot_name = "mbalmayo001",
  extract_individuals = TRUE,
  extract_traits = TRUE
)

# Individuals now include trait columns
names(trees_with_traits$individuals)
```

### Including Individual Features

Add tree-level measurements (e.g., leaf samples, wood density):

```{r with-features}
trees_with_features <- query_plots(
  plot_name = "mbalmayo001",
  extract_individuals = TRUE,
...
species_trees <- query_plots(
  plot_name = "mbalmayo001",
  id_tax = c(12345, 67890),  # Taxon IDs
  extract_individuals = TRUE
)
```
...
The `keep_patterns` and `remove_patterns` options in style configurations allow customization:
...
- **Taxonomic queries**: See `?query_taxa` for taxonomic matching and synonym resolution
- **Trait queries**: See `?query_taxa_traits` for species-level trait data
- **Updates**: See `?add_sp_traits_measures` for adding new measurements
